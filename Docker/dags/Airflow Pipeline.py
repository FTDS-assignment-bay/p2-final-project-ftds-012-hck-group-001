...
This function is utilized to upload CSV data obtained from a data source and store it into PostgreSQL.
    ...
    #read CSV 
    df = pd.read_csv('/opt/airflow/dags/car_insurance.csv')
    #saving csv into postgreSQL
    df.to_sql('dirty_table', conn, index=False, if_exists='replace')  # M
...
def ambil_data():
    ...
    df = pd.read_sql_query("select * from dirty_table", conn)
    df.to_csv('/opt/airflow/dags/dirty_car_insurance.csv', sep=',', index=False)
...
def preprocessing(): 
    ...
    '''
    # data loading
    data = pd.read_csv("/opt/airflow/dags/dirty_car_insurance.csv")
    ...
    #change datatype
    for x in ['vehicle_ownership','married','children','annual_mileage','outcome']:
        data[x] = data[x].astype('int64')
    #data saving
    data.to_csv('/opt/airflow/dags/clean_car_insurance.csv', index=False)
...
This function is utilized to upload CSV data obtained from a data source and store it into PostgreSQL.
    ...
    conn = engine.connect()
    #saving CSV from postgre database
    df = pd.read_csv('/opt/airflow/dags/clean_car_insurance.csv')
